...
public class EntityFriendlyEnderman extends EntityFriendlyMob
{
    ...
    public EntityFriendlyEnderman(World par1World)
    {
        super(par1World);
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isFollowFlag() && !this.worldObj.isRemote){
        	EntityPlayer followTarget=worldObj.getClosestPlayerToEntity(this,60);
        	if (followTarget!=null && this.getDistanceToEntity(followTarget)>=10){
        		this.teleportToEntity(followTarget);
        	}
        }
        ...
        {
        	if (this.isEscortFlag()) findTargetToEscort();
            if (this.entityToAttack != null)
            {
    ...
    protected boolean findTargetToEscort() {
        ...
        Entity tmpTarget=var1.getLastAttackingEntity();
        if (tmpTarget==null){
        	tmpTarget=var1.getAITarget();
        }
        if (tmpTarget==null) return false;
        entityToAttack=tmpTarget;
        return true;
	...
	public void shadowInit(EntityLiving motionTarget) {
		if (!(motionTarget instanceof EntityEnderman)) return;
		EntityEnderman motionEnder=(EntityEnderman)motionTarget;
		this.setCarried(motionEnder.getCarried());
		this.setCarryingData(motionEnder.getCarryingData());
	}

	public void revInit(EntityLiving revTarget) {
		if (!(revTarget instanceof EntityEnderman)) return;
		EntityEnderman targetEnder=(EntityEnderman)revTarget;
		targetEnder.setCarried(this.getCarried());
		targetEnder.setCarryingData(this.getCarryingData());
	}
    static
    {
